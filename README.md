...
Estimated number of clusters: High Risk	51638, Medium Risk	51412, Low Risk	50986
Noise points: Automatically detected by the algorithm. Noise points were excluded from Silhouette Score computation to evaluate cluster cohesion and separation only among core and border points identified by DBSCAN. A high Silhouette Score indicates strong separation between accident-prone zones and well-defined cluster structure, supporting the effectiveness of density-based clustering for this problem.
...
To demonstrate the workflow and results, a preprocessed subset (`accidents_final_states.csv`) containing only relevant features is included. This dataset has gone through scaling, DBSCAN clustering, and addition of a `State` column mapped from latitude and longitude coordinates, and is provided to allow reproducibility of the pipeline and visualization steps.  
